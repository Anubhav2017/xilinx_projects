...
typedef struct {
    ...
    XFloat_vadd_uio_map maps[ MAX_UIO_MAPS ];
} XFloat_vadd_uio_info;

/***************** Variable Definitions **************************************/
static XFloat_vadd_uio_info uio_info;
...
static int uio_info_read_name(XFloat_vadd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFloat_vadd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFloat_vadd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFloat_vadd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFloat_vadd_Initialize(XFloat_vadd *InstancePtr, const char* InstanceName) {
	XFloat_vadd_uio_info *InfoPtr = &uio_info;
