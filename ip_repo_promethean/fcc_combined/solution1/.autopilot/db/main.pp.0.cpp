...
__attribute__((sdx_kernel("fcc_combined", 0))) void fcc_combined(fixed_t* x, fixed_t* dx, fixed_t* wt, fixed_t* dwt, fixed_t* b, fixed_t* db, fixed_t y[50], fixed_t dy[50], int xdim, int ydim, bool fwprop){_ssdm_SpecArrayDimSize(y, 50);_ssdm_SpecArrayDimSize(dy, 50);
#pragma HLS TOP name=fcc_combined
# 9 "fcc_combined/main.cpp"
...
#pragma HLS INTERFACE s_axilite port=return
 ...
 VITIS_LOOP_37_1: for(int i=0;i<ydim;i++){
  bbuf[i]=b[i];
 }

 VITIS_LOOP_41_2: for(int i=0;i<ydim;i++){
  dybuf[i] = dy[i];
 }

 VITIS_LOOP_45_3: for(int i=0;i<xdim;i++){
  xbuf[i] = x[i];
 }

  if (fwprop == true){

   VITIS_LOOP_51_4: for(int i=0;i<xdim;i++){
    dxbuf[i]=0;
   }

   VITIS_LOOP_55_5: for(int i=0;i<ydim;i++){
    VITIS_LOOP_56_6: for(int j=0;j<xdim;j++){
   ...
   LOOP1:for(int i=0; i< ydim;i++){
    ...
    VITIS_LOOP_71_7: for(int i=0;i<ydim;i++){
     y[i] = ybuf[i];
    }
    ...
    VITIS_LOOP_80_8: for(int i=0;i<ydim;i++){
     VITIS_LOOP_81_9: for(int j=0;j<xdim;j++){
      dwbuf[i][j] = dwt[i*xdim+j];
     }
    }

    VITIS_LOOP_86_10: for(int i=0;i<ydim;i++){
         VITIS_LOOP_87_11: for(int j=0;j<xdim;j++){
             dxbuf[j] += dybuf[i] * wbuf[i][j];
             dwbuf[i][j] += dybuf[i]*xbuf[j];
         }
    }

    VITIS_LOOP_93_12: for (int i=0;i<ydim;i++){
        dbbuf[i] += dybuf[i];
    }

    VITIS_LOOP_97_13: for(int i=0;i<ydim;i++){
     VITIS_LOOP_98_14: for(int j=0;j<xdim;j++){
  ...
  VITIS_LOOP_106_15: for(int i=0;i<xdim;i++){
   dx[i]= dxbuf[i];
  }
