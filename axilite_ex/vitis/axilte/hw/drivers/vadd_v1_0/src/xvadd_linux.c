...
int XVadd_Initialize(XVadd *InstancePtr, const char* InstanceName) {
    ...
    // NOTE: slave interface 'Gen' should be mapped to uioX/map0
    InstancePtr->Gen_BaseAddress = (u64)mmap(NULL, InfoPtr->maps[0].size, PROT_READ|PROT_WRITE, MAP_SHARED, InfoPtr->uio_fd, 0 * getpagesize());
    assert(InstancePtr->Gen_BaseAddress);
...
int XVadd_Release(XVadd *InstancePtr) {
    ...
    munmap((void*)InstancePtr->Gen_BaseAddress, InfoPtr->maps[0].size);
