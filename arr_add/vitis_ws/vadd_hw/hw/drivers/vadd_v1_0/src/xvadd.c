...
void XVadd_Set_c(XVadd *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XVadd_WriteReg(InstancePtr->Control_BaseAddress, XVADD_CONTROL_ADDR_C_DATA, (u32)(Data));
    XVadd_WriteReg(InstancePtr->Control_BaseAddress, XVADD_CONTROL_ADDR_C_DATA + 4, (u32)(Data >> 32));
}

u64 XVadd_Get_c(XVadd *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XVadd_ReadReg(InstancePtr->Control_BaseAddress, XVADD_CONTROL_ADDR_C_DATA);
    Data += (u64)XVadd_ReadReg(InstancePtr->Control_BaseAddress, XVADD_CONTROL_ADDR_C_DATA + 4) << 32;
    return Data;
}
