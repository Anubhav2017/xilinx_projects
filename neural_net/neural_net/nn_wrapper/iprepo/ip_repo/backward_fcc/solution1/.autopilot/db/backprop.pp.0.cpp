...
__attribute__((sdx_kernel("backward_fcc", 0))) void backward_fcc(float* x, float* w, float* y, float* b, float* dx, float* dy, float* db, float* dw, int xdim, int ydim){
...
#pragma HLS INTERFACE s_axilite port=ydim
 ...
 VITIS_LOOP_16_1: for(int i=0;i<xdim;i++){
        VITIS_LOOP_17_2: for(int j=0;j<ydim;j++){
            dx[i] = dy[j] * w[i+j*xdim];
        }

    }

    VITIS_LOOP_23_3: for(int i=0;i<ydim;i++){
        VITIS_LOOP_24_4: for(int j=0;j<xdim;j++){
            dw[i*xdim+j] = dy[i]*x[j];
        }
    }


    VITIS_LOOP_30_5: for (int i=0;i<ydim;i++){
        db[i] = dy[i];
    }
}
