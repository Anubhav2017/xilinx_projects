...
entity backward_fcc_control_s_axi is
generic (
...
end entity backward_fcc_control_s_axi;
...
architecture behave of backward_fcc_control_s_axi is
    ...
    constant ADDR_B_DATA_0    : INTEGER := 16#34#;
    constant ADDR_B_DATA_1    : INTEGER := 16#38#;
    constant ADDR_B_CTRL      : INTEGER := 16#3c#;
    constant ADDR_DX_DATA_0   : INTEGER := 16#40#;
    constant ADDR_DX_DATA_1   : INTEGER := 16#44#;
    constant ADDR_DX_CTRL     : INTEGER := 16#48#;
    constant ADDR_DY_DATA_0   : INTEGER := 16#4c#;
    constant ADDR_DY_DATA_1   : INTEGER := 16#50#;
    constant ADDR_DY_CTRL     : INTEGER := 16#54#;
    constant ADDR_DB_DATA_0   : INTEGER := 16#58#;
    constant ADDR_DB_DATA_1   : INTEGER := 16#5c#;
    constant ADDR_DB_CTRL     : INTEGER := 16#60#;
    constant ADDR_DW_DATA_0   : INTEGER := 16#64#;
    constant ADDR_DW_DATA_1   : INTEGER := 16#68#;
    constant ADDR_DW_CTRL     : INTEGER := 16#6c#;
    constant ADDR_XDIM_DATA_0 : INTEGER := 16#70#;
    constant ADDR_XDIM_CTRL   : INTEGER := 16#74#;
    constant ADDR_YDIM_DATA_0 : INTEGER := 16#78#;
    constant ADDR_YDIM_CTRL   : INTEGER := 16#7c#;
    ...
    signal int_db              : UNSIGNED(63 downto 0) := (others => '0');
    signal int_dw              : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_DB_DATA_1 =>
                        rdata_data <= RESIZE(int_db(63 downto 32), 32);
                    when ADDR_DW_DATA_0 =>
                        rdata_data <= RESIZE(int_dw(31 downto 0), 32);
                    when ADDR_DW_DATA_1 =>
                        rdata_data <= RESIZE(int_dw(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_DB_DATA_1) then
                    int_db(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_db(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_DW_DATA_1) then
                    int_dw(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_dw(63 downto 32));
                end if;
            end if;
        end if;
    end process;
