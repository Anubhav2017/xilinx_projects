...
__attribute__((sdx_kernel("forward_fcc", 0))) void forward_fcc(float* x, float* w, float* y, float* b, int xdim, int ydim){
#pragma HLS TOP name=forward_fcc
# 2 "forward_fcc/fwprop.cpp"
...
#pragma HLS INTERFACE s_axilite port=ydim bundle=CTRL
 ...
 VITIS_LOOP_11_1: for(int i=0; i< ydim;i++){
        y[i]= b[i];

        VITIS_LOOP_14_2: for (int j=0; j<xdim;j++){
            y[i]+= w[i*xdim+j]*x[j];
        }
    }
