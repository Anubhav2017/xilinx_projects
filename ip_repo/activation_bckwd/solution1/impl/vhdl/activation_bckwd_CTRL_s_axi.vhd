...
entity activation_bckwd_CTRL_s_axi is
generic (
...
end entity activation_bckwd_CTRL_s_axi;
...
architecture behave of activation_bckwd_CTRL_s_axi is
    ...
    constant ADDR_DX_DATA_0        : INTEGER := 16#1c#;
    constant ADDR_DX_DATA_1        : INTEGER := 16#20#;
    constant ADDR_DX_CTRL          : INTEGER := 16#24#;
    constant ADDR_DY_DATA_0        : INTEGER := 16#28#;
    constant ADDR_DY_DATA_1        : INTEGER := 16#2c#;
    constant ADDR_DY_CTRL          : INTEGER := 16#30#;
    constant ADDR_DIMENSION_DATA_0 : INTEGER := 16#34#;
    constant ADDR_DIMENSION_CTRL   : INTEGER := 16#38#;
    constant ADDR_TYPE_R_DATA_0    : INTEGER := 16#3c#;
    constant ADDR_TYPE_R_CTRL      : INTEGER := 16#40#;
    ...
    signal int_dimension       : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_DIMENSION_DATA_0 =>
                        rdata_data <= RESIZE(int_dimension(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    dimension            <= STD_LOGIC_VECTOR(int_dimension);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_DIMENSION_DATA_0) then
                    int_dimension(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_dimension(31 downto 0));
                end if;
            end if;
        end if;
    end process;
