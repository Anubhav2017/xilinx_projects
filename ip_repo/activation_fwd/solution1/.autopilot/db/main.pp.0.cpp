...
# 3 "activation_fwd/main.cpp" 2



__attribute__((sdx_kernel("activation_fwd", 0))) void activation_fwd(volatile float* in,volatile float* out, int dimension, int type){
#pragma HLS TOP name=activation_fwd
# 6 "activation_fwd/main.cpp"
...
#pragma HLS INTERFACE s_axilite port=type bundle=CTRL
 ...
 memcpy(in_t,(const float*)in,dim*sizeof(float));
    ...
    if(type == 0){
     ...
     VITIS_LOOP_27_1: for(int i=0;i<dim;i++){
#pragma HLS UNROLL factor=5
 if(in_t[i] > 0){
     ...
     VITIS_LOOP_42_2: for(int i=0;i<dim;i++){
#pragma HLS UNROLL factor=5
 out_t[i] = in_t[i];
     }
