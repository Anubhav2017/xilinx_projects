...
typedef struct {
    ...
    XUpdate_weights_uio_map maps[ MAX_UIO_MAPS ];
} XUpdate_weights_uio_info;

/***************** Variable Definitions **************************************/
static XUpdate_weights_uio_info uio_info;
...
static int uio_info_read_name(XUpdate_weights_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XUpdate_weights_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XUpdate_weights_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XUpdate_weights_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XUpdate_weights_Initialize(XUpdate_weights *InstancePtr, const char* InstanceName) {
	XUpdate_weights_uio_info *InfoPtr = &uio_info;
