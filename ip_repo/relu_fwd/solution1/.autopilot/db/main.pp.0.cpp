...
# 3 "relu_fwd/main.cpp" 2

typedef ap_fixed<16,9> fixed;

__attribute__((sdx_kernel("relu_fwd", 0))) void relu_fwd(fixed* x, fixed* y, int dim){
#pragma HLS TOP name=relu_fwd
# 6 "relu_fwd/main.cpp"
...
#pragma HLS INTERFACE s_axilite port=dim bundle=CTRL
 ...
 fixed xbuf[100];
    ...
    VITIS_LOOP_21_1: for(int i=0;i<dim;i++){
        xbuf[i] = x[i];
        if (xbuf[i] > 0){
            ybuf[i] = xbuf[i];
        }
