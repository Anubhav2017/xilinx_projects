...
typedef struct {
    ...
    XLoss_derivative_uio_map maps[ MAX_UIO_MAPS ];
} XLoss_derivative_uio_info;

/***************** Variable Definitions **************************************/
static XLoss_derivative_uio_info uio_info;
...
static int uio_info_read_name(XLoss_derivative_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLoss_derivative_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLoss_derivative_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLoss_derivative_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLoss_derivative_Initialize(XLoss_derivative *InstancePtr, const char* InstanceName) {
	XLoss_derivative_uio_info *InfoPtr = &uio_info;
