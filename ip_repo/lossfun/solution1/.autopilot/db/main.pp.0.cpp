...
__attribute__((sdx_kernel("loss_derivative", 0))) float loss_derivative(float* x,float* dx, int y,int x_size, long int N){
...
#pragma HLS INTERFACE s_axilite port=N bundle=CTRL
 ...
 float log_probs[100];
    ...
    memcpy(xbuff, (const float*)x, x_size*size);

    float loss =0;


    float max = x[0];
    VITIS_LOOP_32_1: for(int i=1;i<x_size;i++){
        if(x[i] > max){
            max = x[i];
        }
    }

    VITIS_LOOP_38_2: for(int i=0;i<x_size;i++){
        log_probs[i] = x[i] - max;
    }

    float sum = 0;

    VITIS_LOOP_44_3: for(int i=0;i<x_size;i++){
        sum += exp(log_probs[i]);
    }

    VITIS_LOOP_48_4: for(int i=0;i<x_size;i++){
        probs[i] = exp(log_probs[i])/sum;
    }

    loss -= log(probs[y]);

    VITIS_LOOP_54_5: for(int i=0;i<x_size;i++){
        if(i == y){
            dx[i] = (probs[i] - 1)/N;
        }
        else{
