...
# 2 "<built-in>" 2
# 1 "relu_bckwd/main.cpp" 2


__attribute__((sdx_kernel("relu_bckwd", 0))) void relu_bckwd(float* x, float* dx, float* dy, int dim){
#pragma HLS TOP name=relu_bckwd
# 3 "relu_bckwd/main.cpp"
...
#pragma HLS INTERFACE s_axilite port=dim bundle=CTRL
 ...
 float xbuf[100];
    ...
    VITIS_LOOP_19_1: for(int i=0;i<dim;i++){
        xbuf[i] = x[i];
        dybuf[i] = dy[i];
    ...
    VITIS_LOOP_31_2: for(int i=0;i<dim;i++){
        dx[i] = dxbuf[i];
    }
