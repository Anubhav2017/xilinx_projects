...
typedef struct {
    ...
    XFcc_combined_uio_map maps[ MAX_UIO_MAPS ];
} XFcc_combined_uio_info;

/***************** Variable Definitions **************************************/
static XFcc_combined_uio_info uio_info;
...
static int uio_info_read_name(XFcc_combined_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFcc_combined_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFcc_combined_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFcc_combined_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFcc_combined_Initialize(XFcc_combined *InstancePtr, const char* InstanceName) {
	XFcc_combined_uio_info *InfoPtr = &uio_info;
    ...
    InstancePtr->Crtl_bus_BaseAddress = (u64)mmap(NULL, InfoPtr->maps[1].size, PROT_READ|PROT_WRITE, MAP_SHARED, InfoPtr->uio_fd, 1 * getpagesize());
