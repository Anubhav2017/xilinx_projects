...
# 2 "<built-in>" 2
# 1 "backward_fcc/backprop.cpp" 2
# 1 "backward_fcc/backprop.h" 1
...
# 4 "backward_fcc/backprop.h" 2



typedef ap_fixed<16,9> fixed;

__attribute__((sdx_kernel("backward_fcc", 0))) void backward_fcc( fixed* x, fixed* w, fixed* dx, fixed* dy, fixed* dw, fixed* db, int xdim, int ydim);
# 2 "backward_fcc/backprop.cpp" 2

__attribute__((sdx_kernel("backward_fcc", 0))) void backward_fcc( fixed* x, fixed* w, fixed* dx, fixed* dy, fixed* dw, fixed* db, int xdim, int ydim){
#pragma HLS TOP name=backward_fcc
# 3 "backward_fcc/backprop.cpp"
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
 ...
 VITIS_LOOP_37_1: for(int i=0;i<ydim;i++){
         VITIS_LOOP_38_2: for(int j=0;j<xdim;j++){
             dwbuf[i][j] = dw[i*xdim+j];
         }
     }

 VITIS_LOOP_43_3: for(int i=0;i<ydim;i++){
      VITIS_LOOP_44_4: for(int j=0;j<xdim;j++){
          wbuf[i][j] = w[i*xdim+j];
      }
 }

 VITIS_LOOP_49_5: for(int i=0;i<ydim;i++){
     VITIS_LOOP_50_6: for(int j=0;j<xdim;j++){
 ...
 VITIS_LOOP_57_7: for (int i=0;i<ydim;i++){
     dbbuf[i] += dybuf[i];
 }
 ...
 VITIS_LOOP_67_8: for(int i=0;i<ydim;i++){
         VITIS_LOOP_68_9: for(int j=0;j<xdim;j++){
             dw[i*xdim+j]=dwbuf[i][j] ;
         }
     }

     VITIS_LOOP_73_10: for(int i=0;i<ydim;i++){
         VITIS_LOOP_74_11: for(int j=0;j<xdim;j++){
             w[i*xdim+j] = wbuf[i][j] ;
         }
     }
