...
#pragma HLS INTERFACE s_axilite port=return bundle=control
 ...
 VITIS_LOOP_46_1: for(int i=0;i<xdim;i++){
  VITIS_LOOP_47_2: for(int j=0;j<ydim;j++){
   dx_t[i] = dy_t[j] * w_t[i+j*xdim];
  }

 }

 VITIS_LOOP_53_3: for(int i=0;i<ydim;i++){
  VITIS_LOOP_54_4: for(int j=0;j<xdim;j++){
   dw[i*xdim+j] = dy_t[i]*x_t[j];
  }
 }


 VITIS_LOOP_60_5: for (int i=0;i<ydim;i++){
  db[i] = dy_t[i];
 }

 VITIS_LOOP_64_6: for(int i=0;i<ydim;i++){
  VITIS_LOOP_65_7: for(int j=0;j<xdim;j++){
