...
#pragma HLS INTERFACE s_axilite port=return bundle=control
 ...
 VITIS_LOOP_43_1: for(int i=0;i<ydim;i++){
         VITIS_LOOP_44_2: for(int j=0;j<xdim;j++){
             dwbuf[i][j] = dw[i*xdim+j];
         }
     }

 VITIS_LOOP_49_3: for(int i=0;i<ydim;i++){
      VITIS_LOOP_50_4: for(int j=0;j<xdim;j++){
          wbuf[i][j] = w[i*xdim+j];
      }
 }
 ...
 VITIS_LOOP_63_7: for (int i=0;i<ydim;i++){
     dbbuf[i] += dybuf[i];
 }
 ...
 VITIS_LOOP_73_8: for(int i=0;i<ydim;i++){
         VITIS_LOOP_74_9: for(int j=0;j<xdim;j++){
             dw[i*xdim+j]=dwbuf[i][j] ;
         }
     }

     VITIS_LOOP_79_10: for(int i=0;i<ydim;i++){
         VITIS_LOOP_80_11: for(int j=0;j<xdim;j++){
             w[i*xdim+j] = wbuf[i][j] ;
         }
     }
