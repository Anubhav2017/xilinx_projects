...
# 2 "<built-in>" 2
# 1 "forward_fcc/fwprop.cpp" 2
# 1 "forward_fcc/fwprop.h" 1
...
# 4 "forward_fcc/fwprop.h" 2
typedef ap_fixed<16,9> fixed;

__attribute__((sdx_kernel("forward_fcc", 0))) void forward_fcc(fixed* x, fixed* w,fixed* y, fixed* b, int xdim, int ydim);
# 2 "forward_fcc/fwprop.cpp" 2



__attribute__((sdx_kernel("forward_fcc", 0))) void forward_fcc(fixed* x, fixed* w,fixed* y, fixed* b, int xdim, int ydim){
#pragma HLS TOP name=forward_fcc
# 5 "forward_fcc/fwprop.cpp"

#pragma HLS INTERFACE s_axilite port=return bundle=CTRL
#pragma HLS INTERFACE m_axi port=x depth=20 offset=slave bundle=gmem
#pragma HLS INTERFACE m_axi port=w depth=20 offset=slave bundle=gmem
#pragma HLS INTERFACE m_axi port=y depth=20 offset=slave bundle=gmem
#pragma HLS INTERFACE m_axi port=b depth=20 offset=slave bundle=gmem
...
#pragma HLS INTERFACE s_axilite port=ydim bundle=CTRL
 ...
 Lread_w: for(int i=0;i<ydim;i++){
    VITIS_LOOP_25_1: for(int j=0;j<xdim;j++){
     wbuf[i][j] = w[i*xdim+j];
    }
